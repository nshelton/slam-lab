...
Status: planned, not implemented. This list turns the workbench feedback into ordered
backend tasks. The current file-based pipeline remains usable; preserve SQLite/NPZ/JSON
and optional Rerun export. The proposed contract is specified in
...
- [ ] Reserve opaque artifact IDs independently of paths and configuration hashes.
- [ ] Add schema/version, type, direct parent ID, producer job ID, payload inventory
  and hashes; include feature artifacts so shared observation provenance is traceable.
- [ ] Add explicit schema/capability detection and a read-only legacy adapter.
- [ ] Require geometry/solve inputs to identify a fixed parent snapshot. Preserve
...
Existing recordings still load, with absent capabilities reported honestly.
...
- [ ] Carry `(matching_artifact_id, matching_row, feature_id)` through every stage.
- [ ] Export `frame_rows`, `source_track_ids` and stable `point_ids` in reconstruction
  NPZ; record source matching and geometry artifact IDs in metadata.
- [ ] Preserve mappings through seed retries, triangulation, BA, point filtering and
  array compaction. Do not reuse point IDs within a job.
- [ ] Keep `observations[:, 1]` as an array-row index; expose the stable ID separately.
- [ ] Treat source tracks as geometry-scoped IDs, not appearance-track IDs. Allow
...
- [ ] Add a minimal job directory with atomically replaced `status.json`; do not wait
  for snapshots or a full workspace manager to support solve monitoring.
- [ ] Separate common lifecycle `state` from stage-specific `phase`; add `job_id`,
  `revision`, `updated_at`, `last_error`, intended artifact ID and worker liveness.
- [ ] Emit registered/total frames, current matching row, current points and genuine
  BA iteration progress when available (otherwise null).
- [ ] Expose `solve-status` and a reader `status()`; preserve legacy matching/geometry
  status via explicit adapters.
- [ ] Report startup, initialization failures, cancellation, publication and exit.
...
- [ ] Store full effective argv as an array, cwd, resolved interpreter/options,
...
- [ ] Share atomic JSON-writing helpers; order payload -> manifest -> job completion.
- [ ] Freeze SQLite with a supported backup/checkpoint strategy, independent of live
...
- [ ] Provide `manifest()`, `list_frames()`, `frame()`, `list_pairs()`, `pair()`,
...
- [ ] Validate schema versions, ancestry, canonical pair/pose direction and cursor
  scope; normalize legacy data in the reader rather than asking the GUI to decode SQL.
- [ ] Add round-trip examples for point selection and job watching to the interface
  document once these APIs actually ship; do not mark proposals as available early.

Acceptance: a GUI reads a legacy run and a new manifest-based run through the same
facade, discovers every committed pair exactly once, and detects unsupported features.
...
2. Job layout/provenance + frozen publication + reader adapters.
